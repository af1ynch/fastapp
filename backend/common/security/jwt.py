...
from backend.common.cdataclasses import AccessToken, NewToken, RefreshToken, TokenPayload
...
def jwt_encode(payload: dict[str, Any]) -> str:
    ...
    :return:
    """
    return cast("str", jwt.encode(payload, settings.TOKEN_SECRET_KEY, settings.TOKEN_ALGORITHM))
...
async def create_new_token(
    refresh_token: str, session_uuid: str, user_id: int, multi_login: bool, **kwargs: Any
) -> NewToken:
    """
    生成新的 token
...
async def get_current_user(db: AsyncSession, pk: int) -> User:
    ...
    admin = await user_dao.get_with_relation(db, user_id=pk)
    if not admin:
        raise errors.TokenError(msg="Token 无效")
    if not admin.status:
...
async def jwt_authentication(token: str) -> GetUserInfoWithRelationDetail:
    ...
    if not cache_user:
        async with async_db_session() as db:
            ...
            admin = GetUserInfoWithRelationDetail(**select_as_dict(current_user))
            await redis_client.setex(
                f"{settings.JWT_USER_REDIS_PREFIX}:{user_id}",
                settings.TOKEN_EXPIRE_SECONDS,
                admin.model_dump_json(),
            )
    else:
        # TODO: 在恰当的时机，应替换为使用 model_validate_json
        # https://docs.pydantic.dev/latest/concepts/json/#partial-json-parsing
        admin = GetUserInfoWithRelationDetail.model_validate(from_json(cache_user, allow_partial=True))
    return admin
