...
from backend.common.cdataclasses import IpInfo, UserAgentInfo
...
async def parse_ip_info(request: Request) -> IpInfo:
    ...
    if settings.IP_LOCATION_PARSE == "online":
        if hasattr(request.headers, "User-Agent"):
            location_info = await get_location_online(ip, request.headers["User-Agent"])
...
def parse_user_agent_info(request: Request) -> UserAgentInfo:
    ...
    :return:
    """
    if not hasattr(request.headers, "User-Agent"):
        return UserAgentInfo(user_agent="Unknown", device=None, os=None, browser=None)
