...
class RoleService:
    ...
    @staticmethod
    async def get(*, pk: int) -> GetRoleWithRelationDetail:
        """
        获取角色详情
        ...
        async with async_db_session() as db:
            ...
            return GetRoleWithRelationDetail.model_validate(role)

    @staticmethod
    async def get_all() -> Sequence[GetRoleDetail]:
        """获取所有角色"""
        async with async_db_session() as db:
            roles = await role_dao.get_all(db)
            return [GetRoleDetail.model_validate(role) for role in roles]
    ...
    @staticmethod
    async def get_menu_tree(*, pk: int) -> list[GetMenuTree | None]:
        """
        获取角色的菜单树形结构
        ...
        async with async_db_session() as db:
            ...
            return [GetMenuTree.model_validate(menu) for menu in menu_tree]  # type: ignore
