...
class UserService:
    ...
    @staticmethod
    async def get_userinfo(*, pk: int | None = None, username: str | None = None) -> GetUserInfoWithRelationDetail:
        """
        获取用户信息
        ...
        async with async_db_session() as db:
            ...
            return GetUserInfoWithRelationDetail.model_validate(user)

    @staticmethod
    async def get_roles(*, pk: int) -> Sequence[GetRoleDetail]:
        """
        获取用户所有角色
        ...
        async with async_db_session() as db:
            ...
            if not user:
                raise errors.NotFoundError(msg="用户不存在")
            return [GetRoleDetail.model_validate(role) for role in user.roles]

    @staticmethod
    async def get_select(
        *, dept: int | None = None, username: str | None = None, phone: str | None = None, status: int | None = None
    ) -> Select:
        """
        获取用户列表查询条件
    ...
    @staticmethod
    async def update_permission(*, request: Request, pk: int, permission_type: UserPermissionType) -> int:
        """
        更新用户权限
    ...
    async def update_password(*, request: Request, obj: ResetPasswordParam) -> int:
        ...
        async with async_db_session.begin() as db:
            ...
            if not user.password:
                raise errors.RequestError(msg="用户密码为空，请先设置密码")
