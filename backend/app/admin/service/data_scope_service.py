...
class DataScopeService:
    ...
    @staticmethod
    async def get(*, pk: int) -> GetDataScopeDetail:
        """
        获取数据范围详情
        ...
        async with async_db_session() as db:
            ...
            return GetDataScopeDetail.model_validate(data_scope)

    @staticmethod
    async def get_all() -> Sequence[GetDataScopeDetail]:
        """获取所有数据范围"""
        async with async_db_session() as db:
            data_scopes = await data_scope_dao.get_all(db)
            return [GetDataScopeDetail.model_validate(item) for item in data_scopes]

    @staticmethod
    async def get_rules(*, pk: int) -> GetDataScopeWithRelationDetail:
        """
        获取数据范围规则
        ...
        async with async_db_session() as db:
            ...
            if not data_scope:
                raise errors.NotFoundError(msg="数据范围不存在")
            return GetDataScopeWithRelationDetail.model_validate(data_scope)
