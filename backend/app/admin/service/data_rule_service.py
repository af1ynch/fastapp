...
class DataRuleService:
    ...
    @staticmethod
    async def get(*, pk: int) -> GetDataRuleDetail:
        """
        获取数据规则详情
        ...
        async with async_db_session() as db:
            ...
            return GetDataRuleDetail.model_validate(data_rule)
    ...
    async def get_columns(model: str) -> list[GetDataRuleColumnDetail]:
        ...
        model_ins: Type[Base] = dynamic_import_data_model(settings.DATA_PERMISSION_MODELS[model])

        model_columns = [
            GetDataRuleColumnDetail(key=column.key, comment=column.comment)
            for column in model_ins.__table__.columns  # type: ignore
    ...
    @staticmethod
    async def get_all() -> Sequence[GetDataRuleDetail]:
        """获取所有数据规则"""
        async with async_db_session() as db:
            data_rules = await data_rule_dao.get_all(db)
            return [GetDataRuleDetail.model_validate(item) for item in data_rules]
