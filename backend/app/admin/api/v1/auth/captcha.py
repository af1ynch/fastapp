...
async def get_captcha(request: Request) -> ResponseSchemaModel[GetCaptchaDetail]:
    ...
    """
    img_byte: Literal["file", "bytesio", "base64"] = "base64"
    img, code = await run_in_threadpool(img_captcha, img_byte=img_byte)
    uuid = uuid4_str()
    ...
    data = GetCaptchaDetail(uuid=uuid, img_type=img_byte, image=img)
