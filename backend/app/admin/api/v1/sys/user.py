...
async def get_current_user(request: Request) -> ResponseSchemaModel[GetCurrentUserInfoWithRelationDetail]:
    data = GetCurrentUserInfoWithRelationDetail.model_validate(request.user)
...
async def get_user_roles(
    pk: Annotated[int, Path(description="用户 ID")],
) -> ResponseSchemaModel[Sequence[GetRoleDetail]]:
...
async def get_users_paged(
    ...
    page_data: PageData[GetUserInfoWithRelationDetail] = await paging_data(db, user_select)
...
async def update_user_permission(
    ...
    count = await user_service.update_permission(request=request, pk=pk, permission_type=permission_type)
    if count > 0:
        return response_base.success()
    return response_base.fail()
