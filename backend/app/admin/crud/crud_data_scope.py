...
class CRUDDataScope(CRUDPlus[DataScope]):
    ...
    async def get_with_relation(self, db: AsyncSession, pk: int) -> DataScope | None:
        """
        获取数据范围关联数据
    ...
    async def update_rules(self, db: AsyncSession, pk: int, rule_ids: UpdateDataScopeRuleParam) -> int:
        ...
        if current_data_scope:
            data_scope_list = list(rules.scalars().all())
            current_data_scope.rules = data_scope_list
            return len(data_scope_list)
        return 0
