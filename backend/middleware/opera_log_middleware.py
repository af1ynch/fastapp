...
class OperaLogMiddleware(BaseHTTPMiddleware):
    ...
    async def get_request_args(self, request: Request) -> dict[str, Any] | None:
        ...
        if len(form_data) > 0:
            form_data_dict = {k: v.filename if isinstance(v, UploadFile) else v for k, v in form_data.items()}
            if "multipart/form-data" not in content_type:
                args["x-www-form-urlencoded"] = await self.desensitization(form_data_dict)
            else:
                args["form-data"] = await self.desensitization(form_data_dict)
